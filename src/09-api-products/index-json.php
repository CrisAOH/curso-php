...
function saveProducts(array $produdcts): void
{
    $path = storagePath();
    file_put_contents($path, json_encode($produdcts, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
    @chmod($path, 0666);
}

function findById(array $products, int $id): ?array
{
    ...
    $fields = [
        ...
        "stock" => [
            ...
            "rules" => function ($value) use (&$errors) {
                ...
                if ((float)$value <= 0) {
                    $errors[] = "El stock debe ser mayor a 0";
                }
            }
        ]
    ];
...
try {
    ...
    if ($method === "GET" && $resourceId !== null) {
        $products = loadProducts();
        $product = findById($products, $resourceId);
        if ($product === null) {
    ...
    if (($method === "PUT" || $method === "PATCH") && $resourceId !== null) {
        ...
        if (array_key_exists("price", $payload)) {
            $updated["price"] = (float)$payload["name"];
        }

        if (array_key_exists("stock", $payload)) {
            $updated["name"] = (float)$payload["name"];
        }
    ...
    if ($method === "DELETE" && $resourceId !== null) {
        ...
        saveProducts($products);
        respondJson(200, ["message" => "Producto eliminado", "data" => $deleted]);
    }
